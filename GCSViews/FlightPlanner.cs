    ...
    public partial class FlightPlanner : MyUserControl, IDeactivate, IActivate
    {
            ...
            for (int a = 0; a < Commands.Rows.Count - 0; a++)
            {
                for (int b = 0; b < Commands.ColumnCount - 0; b++)
                {
                    ...
                    if (cmd < (ushort)MAVLink.MAV_CMD.LAST &&
                        double.Parse(Commands[Alt.Index, a].Value.ToString()) < double.Parse("0"/*TXT_altwarn.Text*/))
                    {
        ...
        public void setfromMap(double lat, double lng, int alt, double p1 = -1)
        {
            ...
            if (alt != -1 && alt != -2 && Commands.Columns[Alt.Index].HeaderText.Equals("Alt"))
            {
                ...
                cell.Value = "100";//TXT_DefaultAlt.Text;
        ...
        public void writeKML()
        {
            ...
            if (TXT_homealt.Text != "" && TXT_homelat.Text != "" && TXT_homelng.Text != "")
            {
                try
                {
                    ...
                    { Tag2 = altmode.Relative.ToString() /*CMB_altmode.SelectedValue.ToString()*/ };
                }
                catch (Exception ex)
                {
                ...
                if ((MAVLink.MAV_MISSION_TYPE)cmb_missiontype.SelectedValue == MAVLink.MAV_MISSION_TYPE.MISSION)
                {
                    ...
                    try
                    {
                        ...
                        wpOverlay.CreateOverlay(home,
                            commandlist,
                            double.Parse(startupWPradius /*TXT_WPRad.Text*/) / CurrentState.multiplieralt,
                            double.Parse("30" /*TXT_loiterrad.Text*/) / CurrentState.multiplieralt, CurrentState.multiplieralt);
                    }
        ...
        {
            e.Row.Cells[Frame.Index].Value = altmode.Relative;//CMB_altmode.SelectedValue;
        ...
        public void Commands_RowsAdded(object sender, DataGridViewRowsAddedEventArgs e)
        {
            ...
            {
                cellFrame.Value = altmode.Relative;//CMB_altmode.SelectedValue;
            }
        ...
        private void Commands_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (Commands.Rows[selectedrow].Cells[Alt.Index].Value != null && Commands.Rows[selectedrow].Cells[Alt.Index].Value.ToString() == "0") Commands.Rows[selectedrow].Cells[Alt.Index].Value = 100;
                }
...
Column 1: Field type (RALLY is the only one at the moment -- may have RALLY_LAND in the future)
 ...
 Column 7: Flags (just 0 for now, also future use).
                ...
                if (file != "" && result == DialogResult.OK)
                {
                    ...
                    try
                    {
                        if (file.EndsWith(".mission"))
                        {
                            ...
                            var format = MissionFile.ConvertFromLocationwps(list, (byte)altmode.Relative);
        ...
        private void saveWPsFast(IProgressReporterDialogue sender)
        {
            ...
            //Establece Home Manual
            MainV2.comPort.doCommand((byte)MainV2.comPort.sysidcurrent, (byte)MainV2.comPort.compidcurrent, MAVLink.MAV_CMD.DO_SET_HOME, 0, 0, 0, 0, (float)home.lat, (float)home.lng, home.alt);
        ...
        {
            string altstring = "100";// TXT_DefaultAlt.Text;
...
//                    MainV2.comPort.MAV.cs.battery_kmleft);
        ...
        public void TXT_homelat_Enter(object sender, EventArgs e)
        {
            ...
            sethome = true;
            //Para desactivar el modo polygon si estas en Home mode.
            polygongridmode = false;
                        ...
                        if (int.TryParse(midline.next.Tag, out pnt2))
                        {
                                     ...
                                     MAVLink.MAV_MISSION_TYPE.MISSION)
                            {
                                ...
                                InsertCommand(pnt2 - 1, MAVLink.MAV_CMD.WAYPOINT, 0, 0, 0, 0,
                                    CurrentMidLine.Position.Lng,
                                    CurrentMidLine.Position.Lat, 100);
                            }
                        }
                    }
        ...
        private void BUT_Poly_Mode_Click(object sender, EventArgs e)
        {
            //Entrar el modo polygon
            addPolygonPointToolStripMenuItem_Click(sender, e);
        }

        private void BUT_WP_Mode_Click(object sender, EventArgs e)
        {
            polygongridmode = false;
        }

        private void BTU_Set_Home_On_Vehicle_Click(object sender, EventArgs e)
        {
            label4_LinkClicked(sender, null);
        }

        private void BUT_Clear_KML_Click(object sender, EventArgs e)
        {
        ...
        void LoadTiffBackground(IProgressReporterDialogue sender)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < (int)geoTiffMetaData.Height_px; i++)
                {
                    ...
                    {
                        int A = Tiff.GetA(buff32[index]);
                        int R = Tiff.GetR(buff32[index]);
                        int G = Tiff.GetG(buff32[index]);
                        int B = Tiff.GetB(buff32[index]);
                        if (R >= 254 && G >= 254 && B >= 254)
                            A = 0;
                        imageTiff.SetPixel(j, i, Color.FromArgb(A, R, G, B));
                        index++;
                    }
        ...
        private void UpdateTiffOverlay()
        {
            if (imageTiff != null)
            {
                ...
                double width =
                (MainMap.MapProvider.Projection.GetDistance(MainMap.FromLocalToLatLng(0, 0),
                    MainMap.FromLocalToLatLng(MainMap.Width, 0)) * 1000.0);

                //Alto en m del screen MainMap
                double height =
                    (MainMap.MapProvider.Projection.GetDistance(MainMap.FromLocalToLatLng(0, 0),
                        MainMap.FromLocalToLatLng(MainMap.Height, 0)) * 1000.0);

                //Res en m/pixel
                double m2pixelwidth = width / MainMap.Width;
                double m2pixelheight = height / MainMap.Height;

                //Calcular el nuevo tamaño del Marker segun la nueva res del mapa.
                tiffMarker.Size = new Size((int)(geoTiffMetaData.Width_m / m2pixelwidth), (int)(geoTiffMetaData.Height_m / m2pixelheight));
        ...
        {
            Color colorNormal = Color.FromArgb(148, 193, 31);
            Color colorActive = Color.FromArgb(207, 185, 23);

            //Esta funcion es para actualizar el color de los botones segun el modo.
            if (sethome)
            {
                BUT_Poly_Mode.BackColor = colorNormal;
                BUT_WP_Mode.BackColor = colorNormal;
                BUT_Set_Home_On_Map.BackColor = colorActive;
            }
            else if (polygongridmode)
            {
                BUT_Poly_Mode.BackColor = colorActive;
                BUT_WP_Mode.BackColor = colorNormal;
